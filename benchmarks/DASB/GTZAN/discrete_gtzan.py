...
class DiscreteSSL_GTZAN(nn.Module):
    ...
    def forward(
        ...
        lens = []
        with torch.no_grad():
            for i, (layer_num, model, vocabulary) in enumerate(zip(self.ssl_layer_ids, self.kmeans_models, self.vocabularies)):
                if layer_num not in SSL_layers:
                    continue
                h_reshaped = h[layer_num]
                ...
                embeddings.append(self.lin_tfs[i](embs_tensor))
            ...
            max_len = max(lens)
            for layer_num, _ in enumerate(self.ssl_layer_ids):
