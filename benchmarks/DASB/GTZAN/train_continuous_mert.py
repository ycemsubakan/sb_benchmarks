...
class ESC50Brain(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        lens = torch.ones(batch[0].shape[0], device=self.device)
        ...
        with torch.no_grad():
            embeddings = self.hparams.embedding_model(wavs)
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        uttid = batch[1].to(self.device)
        classid = batch[1].to(self.device)
        lens = torch.ones(batch[0].shape[0], device=self.device)
    ...
    def on_stage_start(self, stage, epoch=None):
        ...
        self.loss_metric = sb.utils.metric_stats.MetricStats(
                metric=lambda x, y: torch.nn.functional.cross_entropy(x, y).unsqueeze(0)
        )
...
class DiscreteSSL_ESC50(nn.Module):
    ...
    def load_kmeans(
        ...
        assert (
            len(layer_ids) > 0
        ), f"There is no trained k-means model available for *_k{num_clusters}_L*"
...
if __name__ == "__main__":
    ...
    test_pairs = [(path, all_genres.index(path.split('/')[-2])) for path in paths if 90 <= int(path.split('.')[-2]) < 100]
    ...
    class GTZAN(Dataset):
        ...
        def __getitem__(self, idx):
            wav, fs = torchaudio.load(self.pairs[idx][0])
            return (wav.squeeze(), torch.tensor([self.pairs[idx][1]]))
    
    def collate_fn(batch):
        """This function defines how we will """

        wavs_list = [els[0] for els in batch]
        labels_list = [els[1] for els in batch]

        nested_wavs = torch.nested.nested_tensor(wavs_list)
        wavs_padded = nested_wavs.to_padded_tensor(0)
        return wavs_padded, torch.tensor(labels_list)
    tr_data = GTZAN(tr_pairs)
    valid_data = GTZAN(valid_pairs)
    test_data = GTZAN(test_pairs)
    tr_loader = DataLoader(tr_data, collate_fn=collate_fn, batch_size=hparams['batch_size'], shuffle=True)
    valid_loader = DataLoader(valid_data, collate_fn=collate_fn, batch_size=hparams['batch_size'])
    test_loader = DataLoader(test_data, collate_fn=collate_fn, batch_size=hparams['batch_size'])
